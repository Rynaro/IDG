...
The installer creates `.opencode/agents/scribe.md`.

## 2. Config

The installed agent descriptor at `.opencode/agents/scribe.md`:
...
Expected: OpenCode loads the Scribe agent from `.opencode/agents/scribe.md`, which points to `agents/scribe/agent.md`. Scribe runs the IDG cycle and delivers an ADR with provenance metadata.

## 4. Troubleshooting

**Agent not visible in OpenCode**
- Verify `.opencode/agents/scribe.md` is present
- Restart the OpenCode session after adding the agent file

**Scribe not finding skill files**
- The `agents/scribe/` directory must be at the project root
